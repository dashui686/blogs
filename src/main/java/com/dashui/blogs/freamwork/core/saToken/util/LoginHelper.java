...
import com.dashui.blogs.freamwork.core.saToken.enums.UserType;
...
public class LoginHelper {
     ...
     * @param model     配置参数
     */
    public static void login(LoginAdmin loginAdmin, SaLoginModel model) {
        SaStorage storage = SaHolder.getStorage();
        storage.set(LOGIN_USER_KEY, loginAdmin);
        storage.set(USER_KEY, loginAdmin.getId());
        storage.set(GROUP_KEY, loginAdmin.getGroupId());
        model = ObjectUtil.defaultIfNull(model, new SaLoginModel());
        StpUtil.login(loginAdmin.getId(),
        model.setExtra(USER_KEY, loginAdmin.getId()).setExtra(GROUP_KEY, loginAdmin.getGroupId()));
        SaSession tokenSession = StpUtil.getTokenSession();
        tokenSession.updateTimeout(model.getTimeout());
        tokenSession.set(LOGIN_USER_KEY, loginAdmin);
    }
    ...
    public static LoginAdmin getLoginAdmin() {
        return (LoginAdmin) getStorageIfAbsentSet(LOGIN_USER_KEY, () -> {
            SaSession session = StpUtil.getTokenSession();
            if (ObjectUtil.isNull(session)) {
                return null;
            }
    ...
    public static Long getGroupId() {
        return Convert.toLong(getExtra(GROUP_KEY));
    }
