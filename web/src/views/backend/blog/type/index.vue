<template>
  ...
  <script lang="ts" setup name="BlogType">
  import { getById,getTypeList,insertType,deleteType,updateType } from '@/api/backend/blog/type';
  ...
  const submitForm = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate((valid, fields) => {
      if (valid) {
        ...
        (!form.value.id?insertType(form.value):updateType(form.value))
        .then(e=>{
            ElNotification({
                message: e.msg,
                type: 'success',
